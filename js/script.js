...
const showNewsFirst = (datas, name) => {
  ...
  data.sort(
    (firstItem, secondItem) => secondItem.total_view - firstItem.total_view
  );
  newsAdd.innerHTML = ``;
  newsShow(data);
};
        ...
        <div class="row g-2">
            ...
            <div class="col-12 col-md-8">
                <div class="card-body">
                    ...
                    <p class="card-text text-secondary mb-5">${data.details.slice(
                      0,
                      200
                    )}</p>
                    <p class="card-text text-secondary mb-5">${data.details.slice(
                    ...
                    <div class="newsDetails">
                        <div class="newsAuthor">
                            <img src="${
                              data.author.img
                            }" width="50px" height="50px" class="rounded-circle mx-2" alt="...">
                    ...
                    <div>
                        <i class="fa-regular fa-eye mx-2"></i>${
                          data.total_view ? data.total_view : "Not available"
                        }
                    </div>
                    <div>
                        <button onclick="getNewsDetails('${
                          data._id
                        }')" type="button" data-bs-toggle="modal" data-bs-target="#exampleModal"  class="btn btn-primary px-3 mt-4">Author Details</button>
                    </div>
                </div>
            </div>
        </div>
...
const showDetails = (data) => {
  ...
  detailAuthorName.innerText = `${
    data.author.name ? data.author.name : "Not available"
  }`;
  const detailPublishDate = document.getElementById("detailPublishDate");
  detailPublishDate.innerText = `${
    data.author.published_date ? data.author.published_date : "Not available"
  }`;
